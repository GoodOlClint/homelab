...
| `make adguard-rewrite DOMAIN=<fqdn> ANSWER=<ip>` | Add a DNS rewrite on every resolver instance via the API (the config template is initial-only) — no-op when present |
