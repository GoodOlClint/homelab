...
**Alerting** runs in two lanes with one shared ntfy channel ([ADR 0011](docs/decisions/0011-two-alerting-lanes-uptime-kuma-for-reachability-alertmanager-for-metrics.md)): Uptime Kuma owns internal reachability (on the cluster at `uptime-kuma.<domain>`, hand-managed UI config, reproducible via `scripts/seed_uptime_kuma.py` run inside a pod mounting the `uptime-kuma` PVC, inventory in [docs/uptime-kuma-monitors.md](docs/uptime-kuma-monitors.md)); Prometheus + Alertmanager own everything metric-shaped. UptimeRobot stays the external outside-in prober. The ntfy topic is the credential and lives at Infisical `/monitoring/alert_ntfy_topic`.
...
| `/docker` | — (values pre-exist; `kubernetes/games/` consumes via `InfisicalSecret`, no generator since P4d) | k8s `games` (InfisicalSecret) | valheim_server_password, valheim_supervisor_password (authentik_* are dead) | valheim_discord_webhook (optional — absent today, the join-code hook stays inert until it exists) |
...
| `/homepage` | control (portainer_api_key only), adguard (adguard_*) | k8s `homepage` (InfisicalSecret) | portainer_api_key, adguard_* | unifi_*, authentik_token |
...
| `make adguard-rewrite DOMAIN=<fqdn> ANSWER=<ip>` | Add or repoint a DNS rewrite on every resolver instance via the API (the config template is initial-only) — no-op when present, stale answers for the domain are deleted |
...
| `make talos-ingress` / `talos-infisical` / `infisical-smoke` / `talos-homepage` | P4a (ADR 0035): Traefik + wildcard cert on offset 65; Infisical operator + the bootstrap `infisical-universal-auth` secret (smoke = sync one `/shared` key and delete it); homepage Deployment/ConfigMap/InfisicalSecrets/Ingress + AdGuard rewrites for `homepage.<domain>` and `home.<services zone>` |
| `make talos-plex-services` / `plex-services-migrate FROM=<ip>` / `plex-services-kuma` / `plex-pbs-image` | P4c (ADR 0037): the arr stack (`kubernetes/plex-services/`, NFS media PV, 8 UI hostnames on the ingress); one-off state copy from the old guest; Kuma row repoint; build+push the `proxmox-backup-client` image for the pg-backup CronJob |
| `make talos-games` / `games-migrate FROM=<ip>` / `games-kuma` | P4d (ADR 0038): Valheim + PlayFab sidecar on MetalLB offset 67 + Kiwix over NFS; player-gated one-off state copy from the old guest; Kuma row repoint |
