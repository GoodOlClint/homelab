...
`tls = "self"` additionally makes the guest run a **public-CA** certbot lane: `cert_client` with `cert_client_issuer: letsencrypt` (Let's Encrypt + `python3-certbot-dns-cloudflare`, a per-consumer `cert_client_cloudflare_token` defaulting to `/infrastructure/cloudflare_dns_api_token`) — the switch ADR 0049 added and `plex_certificate` already consumes (#36, verified 2026-09-10). A publicly-trusted certificate must be validated at the **public** authoritative nameserver — Cloudflare — never at BIND.
...
| **WP4** | `cert_client` with `cert_client_issuer: letsencrypt` on the Case guests (the role already exists, #36). Prove on **one throwaway guest on worklab**, never first on a Case VM. | `openssl s_client` returns `issuer=...Let's Encrypt`; a forced renewal succeeds (`certbot renew --force-renewal --no-random-sleep-on-renew --cert-name <fqdn>`). Never `--dry-run` — it silently retargets LE staging. |
