...
| Nodes | BIOS: **Secure Boot ON** (ADR 0026), UEFI network stack / PXE on the install NIC enabled, boot order local disk first — PXE is invoked one-shot via AMT *Reset to PXE*. |
...
- End-to-end: AMT *Reset to PXE* on a node **with Secure Boot enabled** → unattended install with no media session → `ssh root@<node> hostname` returns the node name within 15 min of the reset, `mokutil --sb-state` = enabled, `nicN` names pinned, OSD disk untouched. Fails today (no netboot infrastructure), passes after.
