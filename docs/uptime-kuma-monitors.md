...
| Valheim — PlayFab lobby | JSON query | `valheim-status.games.svc.cluster.local:8081/status.json`, `online == true` (in-cluster sidecar keyed on the server name, ADR 0038; re-pointed 2026-08-24 — the 2026-08-23 note claimed a `:8081` repoint that never reached the DB, the row was DOWN against `:8080` `platform`) | 60s |
...
- **Valheim is monitored over HTTP, not on its game port.** The game port is UDP and answers no query — the A2S responder does not exist under crossplay, because `ZNet.OpenServer()` only creates the Steam game server on the Steamworks backend. What Kuma watches instead is the `valheim-status` sidecar's `status.json` (`:8081` in the `games` pod, rewritten every 60s from a PlayFab lobby query by server name). The monitor asserts `online == true`; a lost lobby writes `online: false` (red), and an API error leaves the file untouched. This proves the game server is registered with PlayFab; it does not prove a player can traverse the VPS relay to reach it.
