...
| Key Name | `acme-pfsense` |
| Key Algorithm | `HMAC-SHA256` |
| Key | Infisical `/infrastructure/acme_tsig_pfsense` (its own key, ADR 0050) |
...
The Key Name help text ("(Optional) A name for the key, if it is different than `_acme-challenge.[DomainName]`") is misleading — it is the **TSIG key name** and is required. BIND's policy is `grant acme-pfsense name _acme-challenge.pfsense.<service domain>. TXT` (ADR 0050), so that key may write exactly its own challenge record and nothing else; a refused update means the request was for any other name.
