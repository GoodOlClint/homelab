...
|---|---|---|---|---|---|
| — | ~~Re-key the three hand-enrolled ACME clients to their own TSIG keys (ADR 0050, 2026-09-10): pfSense (`acme-pfsense`), the DSM (`acme-ds1821plus`, `~/.acme.sh/acme-tsig.key`), Home Assistant (`acme-homeassistant`, the add-on's `rfc2136_*`) — the shared `acme-key` is gone from BIND, so their next renewal fails until this is done; then delete `/infrastructure/acme_tsig_key_secret` | pfSense, DSM, HA | a forced renewal on each succeeds; `infisical secrets get acme_tsig_key_secret --path /infrastructure` is not found | **HUMAN** | [pfsense-acme.md](pfsense-acme.md), [synology-acme.md](synology-acme.md), [homeassistant-acme.md](homeassistant-acme.md) carry the new key names | **DONE 2026-09-10** (operator; verified: `/infrastructure` carries `acme_tsig_pfsense`/`acme_tsig_ds1821plus`/`acme_tsig_homeassistant` and no `acme_tsig_key_secret`) |
| — | homelab-mcp deploy key: add the `mcp-deploy` public key (`ssh-keygen -y -f /var/lib/mcp/.ssh/id_ed25519` on the mcp VM, private half in Infisical `/mcp/github_deploy_key`) on GitHub → `GoodOlClint/homelab-mcp` → Settings → Deploy keys, **read-only**; then `make ansible mcp`. Repeat only if the key is regenerated (a rebuild keeps it — it lives in the vault) | GitHub | `make ansible mcp` installs the gateway; `curl https://mcp.<service domain>/healthz` → 200 | **HUMAN** | ADR 0052 consequence note (2026-09-14): a deploy key over the App token or a PAT; GitHub's deploy-key API needs repo-admin rights the agent App does not hold | **DONE 2026-09-14** (operator; `make ansible mcp` installed v0.2.0) |
| — | pfSense `mcp` user privileges: the three REST API v2 GET privileges (Firewall Rules, Status Gateways, VPN WireGuard Peers) and no `admins` group | pfSense | the mcp VM's key gets 200 on `/api/v2/status/gateways` and 403 on `/api/v2/users` | **HUMAN** | pfSense privileges are hand-managed (ADR 0005) |
...
| Application UIs: Plex claim/library, Jellyfin users, authentik invitations, MeshCentral first account, Libation Audible login, UptimeRobot monitors | per-app docs | [authentik-setup.md](authentik-setup.md), [uptimerobot-setup.md](uptimerobot-setup.md), [libation-audiobook-sync-plan.md](libation-audiobook-sync-plan.md) | First-run identity steps only; configuration is API-driven where it exists (Jellyfin, Kuma, homepage, the arr stack) | **HUMAN** for OAuth/device logins (Audible, Plex claim, passkeys); **AUTO-NEW** for UptimeRobot (a terraform provider exists; three monitors) |
