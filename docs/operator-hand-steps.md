...
|---|---|---|---|---|---|
| — | Re-key the three hand-enrolled ACME clients to their own TSIG keys (ADR 0050, 2026-09-10): pfSense (`acme-pfsense`), the DSM (`acme-ds1821plus`, `~/.acme.sh/acme-tsig.key`), Home Assistant (`acme-homeassistant`, the add-on's `rfc2136_*`) — the shared `acme-key` is gone from BIND, so their next renewal fails until this is done; then delete `/infrastructure/acme_tsig_key_secret` | pfSense, DSM, HA | a forced renewal on each succeeds; `infisical secrets get acme_tsig_key_secret --path /infrastructure` is not found | **HUMAN** | [pfsense-acme.md](pfsense-acme.md), [synology-acme.md](synology-acme.md), [homeassistant-acme.md](homeassistant-acme.md) carry the new key names |
