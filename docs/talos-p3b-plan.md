...
Status: **Built 2026-08-23** (self-signed CA is a placeholder for Infisical PKI — see ADR 0034). Deviations: MetalLB needs `speaker.ignoreExcludeLB` (all nodes are control planes); workflow `runs-on` had to become the scale-set name; PSProxmoxVE's cleanup discovered the node via `/nodes` (invisible to the pool token) — fixed in PR #84; the `ci_api_token` output already carries the `ci@pve!ci=` prefix. Decision record: [ADR 0034](decisions/0034-p3b-metallb-l2-on-a-reserved-vlan40-slice-zot-behind-an-internal-cert-manager-ca-arc-scale-sets-as-hostnetwork-dind-pods-pinned-to-one-node-and-a-cronjob-reaper.md). Follows [P3a](talos-p3a-plan.md) / [ADR 0033](decisions/0033-talos-p3a-rides-talosctl-from-a-kubernetes-tree-with-the-factory-image-pinned-in-cloud-images-the-worklab-member-via-a-second-bpg-provider-alias-flannel-and-cluster-secrets-in-infisical.md); builds [ADR 0022](decisions/0022-container-images-pull-through-a-fleet-local-registry-via-templated-refs-the-registry-rebuilds-itself-from-upstream.md) (registry) and [ADR 0032](decisions/0032-ci-runners-are-ephemeral-and-build-their-own-guests-inside-a-pve-pool-on-an-isolated-vlan-and-the-mac-studio-is-a-separate-trust-tier.md) (CI sandbox).
