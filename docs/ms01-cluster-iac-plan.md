...
- **Fleet apt source is flaky — decide before WP4 lands the rebuilt fleet.** Cloud-init leaves every guest on `archive.ubuntu.com`, whose IPv4 path from here is erratic: measured 2026-08-04 from unifi, three consecutive 6 MB range fetches gave 0 B/s / 4.5 MB/s / 0 B/s, against 8.9 / 9.2 / 3.9 MB/s to `mirror.math.princeton.edu`. Not VLAN-specific (a vlan40-egress guest measured slower than vlan10-only unifi) and not MTU (PMTU 1462 discovered correctly). Guests are IPv4-only on mgmt, so unlike a dual-stack client they cannot fall back to Canonical's much faster IPv6. Symptom is a silent multi-minute stall in `make update` whenever a kernel is in the set — apt has no minimum-rate abort, so a 19 KB/s transfer looks identical to a hang. Candidates: a **local apt cache/mirror** in the fleet (apt-cacher-ng or a pull-through, which also cuts WAN pulls across ~15 guests), or a deb822 multi-URI stanza listing a fast mirror ahead of `archive.ubuntu.com` as fallback. Decide before WP4; set it in cloud-init so fresh guests inherit it.
