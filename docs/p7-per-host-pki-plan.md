...
- **`make plan` and docker guests:** the Infisical VM's Caddy container recreates a veth on every compose restart, which the guest agent reports as a new interface (link-local + MAC) → "Changes to Outputs" only. `make refresh` (new target) settles it; no resource ever changes.
...
- Leftovers for the operator: the stray empty "Certificate Manager" project from 2026-08-21 in the org (harmless, active project is `homelab-pki`); a `p7-probe` leaf may remain in the vault's certificate list; the old `.internal`-SAN self-signed certs on the nodes are gone.
