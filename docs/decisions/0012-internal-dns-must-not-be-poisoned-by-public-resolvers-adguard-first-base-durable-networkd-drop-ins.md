...
- **2026-09-15 (consequence):** the principle held — no internal name ever consulted a public resolver — but AdGuard Home serves both through one worker pool, and a Cox outage starved BIND-bound answers behind 20 s public timeouts. The `adguard` role now bounds the upstream timeout at 3 s and sizes the pool at 3000 (measured public p99.9 ~0.7 s). Inverting the chain (guests → BIND → AdGuard) was rejected: AdGuard must see the real client address in its log.
