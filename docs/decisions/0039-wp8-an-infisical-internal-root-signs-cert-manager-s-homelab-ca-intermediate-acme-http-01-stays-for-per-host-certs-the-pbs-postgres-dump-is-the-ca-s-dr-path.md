...
- P7 (2026-08-25, [ADR 0041](0041-p7-per-host-certs-are-acme-dns-01-against-infisical-through-bind-s-dynamic-update-path-on-a-scoped-tsig-key-pbs-fingerprint-pins-retire-infisical-s-own-cert-is-api-issued-behind-caddy.md)): the per-host phase landed as ACME **DNS-01** (Infisical v0.162.24 offers it; "HTTP-01 only" above is superseded) through BIND's dynamic-update path, with an RSA intermediate `Homelab Hosts CA` under this root as the fleet issuer; the fleet root-distribution role is `ca_trust`.
