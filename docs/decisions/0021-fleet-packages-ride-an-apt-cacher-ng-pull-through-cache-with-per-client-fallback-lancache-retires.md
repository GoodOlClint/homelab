...
- P6 (2026-08-25): the probe rides cloud-init `write_files` (address derived from the apt-cache guest definition in Terraform, `DIRECT` fallback intact) so first-boot packages already use the cache; the `apt_proxy` role writes identical bytes and stays the repair path. Lancache is removed — role, play, tag, README rows; VM 110 no longer existed.
