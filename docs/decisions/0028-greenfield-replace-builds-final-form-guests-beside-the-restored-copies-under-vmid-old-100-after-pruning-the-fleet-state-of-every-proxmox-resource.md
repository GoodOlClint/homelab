...
- SDN on the cluster is the `Homelab` VLAN zone on `vmbr0` only; VLANs the hosts use on that bridge (20 storage, 30 mgmt) carry `sdn_zone: null` and are never VNETs — the first apply proved a VNET for them enslaves the host sub-interface (CLAUDE.md What Never To Do, 2026-08-20).
- The fleet resolver cannot be swapped by stop-then-build: the operator's workstation resolves through it, so `terraform`/`ansible` lose DNS the moment the restored copy stops (hit 2026-08-20 — `terraform init` failed against `registry.terraform.io`). AdGuard is replaced only as the design's LXC pair, second instance serving before the restored copy stops; the first proving guest is homepage.
