...
- `master_ip` is the master's **ring0** address (amended 2026-08-11 — reversing the original mgmt-IP guidance): the module's re-run idempotency check compares `master_ip` against `/cluster/status` node `ip` fields, which are corosync ring0 addresses, so a mgmt `master_ip` makes every re-run fail "already part of a cluster" on the real VLAN 30/31 split (the nested harness masked this because mgmt==ring0 there). pveproxy binds `::`, so the join reaches the master's API on the ring0 VLAN.
