  ...
  docker_interfaces = {
    for idx, vlan_key in local.docker_vlans : vlan_key => {
      ...
      macaddress = format(
        ...
        2, # unique per-VM id, change for each VM file
        idx,
...
resource "proxmox_virtual_environment_file" "docker_user_data" {
  ...
  source_raw {
    data = templatefile("${path.module}/user-data.yaml.tmpl", {
      hostname  = "docker"
      fqdn      = "docker.goodolclint.internal"
    ...
    file_name = "docker-user-data.yaml"
  }
}

resource "proxmox_virtual_environment_file" "docker_network_data" {
  content_type = "snippets"
  datastore_id = var.virtual_environment_storage
  node_name    = var.virtual_environment_node
  ...
  source_raw {
    data = templatefile("${path.module}/network-data.yaml.tmpl", {
      interfaces = [
        for vlan_key, iface in local.docker_interfaces : {
          name       = "eth_${vlan_key}"
          mtu        = iface.mtu
          dhcp       = iface.dhcp
    ...
    file_name = "docker-network-data.yaml"
  }
}
...
resource "proxmox_virtual_environment_vm" "dockerVM" {
  ...
  initialization {
    ...
    network_data_file_id = proxmox_virtual_environment_file.docker_network_data.id
  }
