  ...
  homebridge_interfaces = {
    for idx, vlan_key in local.homebridge_vlans : vlan_key => {
      ...
      macaddress = format(
        ...
        3, # unique per-VM id, change for each VM file
        idx,
...
resource "proxmox_virtual_environment_file" "homebridge_user_data" {
  content_type = "snippets"
  datastore_id = var.virtual_environment_storage
  node_name    = var.virtual_environment_node
  ...
  source_raw {
    data = templatefile("${path.module}/user-data.yaml.tmpl", {
      hostname  = "homebridge"
      fqdn      = "homebridge.goodolclint.internal"
    ...
    file_name = "homebridge-user-data.yaml"
  }
}

resource "proxmox_virtual_environment_file" "homebridge_network_data" {
  content_type = "snippets"
  datastore_id = var.virtual_environment_storage
  node_name    = var.virtual_environment_node
  ...
  source_raw {
    data = templatefile("${path.module}/network-data.yaml.tmpl", {
      interfaces = [
        for vlan_key, iface in local.homebridge_interfaces : {
          name       = "eth_${vlan_key}"
          mtu        = iface.mtu
          dhcp       = iface.dhcp
    ...
    file_name = "homebridge-network-data.yaml"
  }
}
...
resource "proxmox_virtual_environment_vm" "homebridgeVM" {
  ...
  initialization {
    datastore_id = var.primary_disk_storage
    user_data_file_id = proxmox_virtual_environment_file.homebridge_user_data.id
    network_data_file_id = proxmox_virtual_environment_file.homebridge_network_data.id
  }
