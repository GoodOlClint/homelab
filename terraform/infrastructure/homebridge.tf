...
resource "proxmox_virtual_environment_vm" "homebridgeVM" {
  ...
  connection {
   host = proxmox_virtual_environment_vm.homebridgeVM.ipv4_addresses[1][0] # IP address of the host where commands will run
  ...
  provisioner "file" {
    source   = "scripts/installHomebridge.sh"
