...
EOT
  ...
  kubectl -n "$NS" get secret smoke -o jsonpath='{.data}' | jq -r 'keys[]' | grep -q pbs_backup_token || { echo "infisical operator smoke: FAIL" >&2; exit 1; }
echo "infisical operator smoke: PASS"
