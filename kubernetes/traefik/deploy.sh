...
export DOMAIN="$SERVICE_DOMAIN" AUTH_HOST="auth.$SERVICE_DOMAIN" LB_IP="$(subnet_ip 65)" REGISTRY="registry.$(j .domain)" ISSUER="${ISSUER:-letsencrypt}"
...
envsubst '${AUTH_HOST}' < "$HERE/middleware.yaml" | kubectl apply -f -
