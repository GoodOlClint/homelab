...
// Usage: node valheim-playfab-status.mjs --code 123456 | --ip 1.2.3.4:2456 | --entity-id <id>
...
export async function queryValheimPlayFab({ code, ip, entityId }) {
  ...
  const filter = code ? `string_key4 eq '${code}'`
...
async function main() {
  ...
  const key = { code: get('--code'), ip: get('--ip'), entityId: get('--entity-id') };
  if (!key.code && !key.ip && !key.entityId) {
    console.error('usage: valheim-playfab-status.mjs --code <join-code> | --ip <ip:port> | --entity-id <id>');
    process.exit(2);
  }
